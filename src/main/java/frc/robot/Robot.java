...
public class Robot extends LoggedRobot {
    ...
    public void robotInit() {
        NetworkTable frontTable = NetworkTableInstance.getDefault().getTable(Vision.Limelights.FRONT.table);
        NetworkTable rearTable = NetworkTableInstance.getDefault().getTable(Vision.Limelights.REAR.table);
        frontVisionPose = frontTable.getDoubleArrayTopic("botpose_wpiblue").subscribe(new double[] {});
        frontCamera2TagPose = frontTable.getDoubleArrayTopic("targetpose_cameraspace").subscribe(new double[] {});
        rearVisionPose = rearTable.getDoubleArrayTopic("botpose_wpiblue").subscribe(new double[] {});
        rearCamera2TagPose = rearTable.getDoubleArrayTopic("targetpose_cameraspace").subscribe(new double[] {});
        ...
        // cube: LeftBumper(5), tipped cone: RightTrigger(Axis 3), upright cone:
        // RightBumper(6)
        driver
                .leftBumper()
                .onTrue(
        ...
        //
        // LightStrip.Commands.blinkAnyPattern(Pattern.DarkGreen)).until(() ->
        // fourBar.isAtTarget()),
        // new ConditionalCommand(
        // LightStrip.Commands.setColorPattern(Pattern.DarkGreen),
        // new SequentialCommandGroup(
        ...
        operator.y().onTrue(
                new SequentialCommandGroup(
                        // Set target angle
                        new InstantCommand(() -> {
                            slapper.goToAngle(90);
                        })
                                // Until we are at desired target
                                .until(() -> {
                                    return slapper.isAtTarget;
                                }),
                        new InstantCommand(() -> {
