...
public class CustomHolonomicDriveController {
  ...
  private static CustomHolonomicDriveController instance;

  public static CustomHolonomicDriveController getInstance() {
    if (instance == null) {
      instance =
  ...
  public ChassisSpeeds calculate(Pose2d currentPose, FullStateSwerveTrajectoryState state) {
    ...
    Logger.getInstance()
        .recordOutput("Trajectory/Controller/R", poseRef.getRotation().getDegrees());
    Logger.getInstance()
        .recordOutput(
            "Trajectory/Controller/R error",
            currentPose.getRotation().minus(poseRef.getRotation()).getDegrees());

    Logger.getInstance().recordOutput("Trajectory/Controller/R ff", thetaFF);
    Logger.getInstance().recordOutput("Trajectory/Controller/R feedback", thetaFeedback);
